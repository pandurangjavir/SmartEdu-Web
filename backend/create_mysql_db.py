#!/usr/bin/env python3
"""
Create MySQL database and initialize with complete data
"""
...
def create_database():
    ...
    try:
        ...
        # Create database
        cursor.execute("DROP DATABASE IF EXISTS college_system_cse")
        cursor.execute("CREATE DATABASE college_system_cse CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci")
        
        print("Database 'college_system_cse' created successfully!")
...
def init_complete_database():
    """Initialize database with complete schema and all 30 students"""
    ...
    with app.app_context():
        ...
        # Insert sample data
        print("\nInserting complete sample data...")
        ...
        db.session.add_all([sy_cse, ty_cse, final_cse])
        db.session.flush()
        
        # 4. Insert Subjects (5 per class)
        subjects_data = [
            # SY-CSE Subjects
            (sy_cse.class_id, 'Data Structures', 'DS', 4),
            (sy_cse.class_id, 'Computer Networks', 'CN', 4),
            (sy_cse.class_id, 'Database Management Systems', 'DBMS', 4),
            (sy_cse.class_id, 'Operating Systems', 'OS', 4),
            (sy_cse.class_id, 'Discrete Mathematics', 'DM', 4),
            
            # TY-CSE Subjects
            (ty_cse.class_id, 'Web Technologies', 'WT', 4),
            (ty_cse.class_id, 'Software Engineering', 'SE', 4),
            (ty_cse.class_id, 'Theory of Computation', 'TOC', 4),
            (ty_cse.class_id, 'Computer Organization', 'CO', 4),
            (ty_cse.class_id, 'Artificial Intelligence', 'AI', 4),
            
            # Final-CSE Subjects
            (final_cse.class_id, 'Machine Learning', 'ML', 4),
            (final_cse.class_id, 'Big Data Analytics', 'BDA', 4),
            (final_cse.class_id, 'Cloud Computing', 'CC', 4),
            (final_cse.class_id, 'Cyber Security', 'CS', 4),
            (final_cse.class_id, 'Blockchain Technology', 'BT', 4),
        ]
        ...
        # 5. Insert SY-CSE Students (10 students)
        sy_students_data = [
        ...
        # 6. Insert TY-CSE Students (10 students)
        ty_students_data = [
        ...
        # 7. Insert Final-CSE Students (10 students)
        final_students_data = [
